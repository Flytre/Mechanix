...
public abstract class ItemProcessingRecipe<T extends Inventory> implements MechanixRecipe<T> {
    ...
    private final QuantifiedIngredient[] upgrades;

    public ItemProcessingRecipe(Identifier id, QuantifiedIngredient[] inputs, OutputProvider[] outputs, int craftTime, QuantifiedIngredient[] upgrades) {
        this.id = id;
        this.inputs = inputs;
    ...
    @Override
    public QuantifiedIngredient[] getUpgrades() {
        return upgrades;
    }

    /**
     * DO NOT USE ON MULTI-OUTPUT RECIPES
     */
    @Override
    @Deprecated
    ...
    public boolean matches(T inv, World world) {
        ...
        if (getUpgrades().length > 0) {
            if (inv instanceof UpgradeInventory) {
                UpgradeInventory ui = (UpgradeInventory) inv;
                if (ui.hasNoUpgrades() || !Arrays.stream(upgrades).allMatch(i -> ui.getUpgrades().stream().anyMatch(i::test)))
                    return false;
            } else
                return false;
        }
