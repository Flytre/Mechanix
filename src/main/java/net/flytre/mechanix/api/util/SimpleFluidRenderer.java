...
public final class SimpleFluidRenderer {
    ...
    public static void render(Fluid fluid, MatrixStack matrices, int x0, int y0, int width, int height, int zOffset) {
            ...
            if (renderingData != null) {
                ...
                bb.vertex(matrix, x0 + width, y0, zOffset).texture(sprite.getMaxU(), sprite.getMinV()).color(r, g, b, a).next();
                bb.vertex(matrix,x0, y0, zOffset).texture(sprite.getMinU(), sprite.getMinV()).color(r, g, b, a).next();
                bb.vertex(matrix, x0, y0 + height, zOffset).texture(sprite.getMinU(), sprite.getMaxV()).color(r, g, b, a).next();
                bb.vertex(matrix,x0 + width, y0 + height, zOffset).texture(sprite.getMaxU(), sprite.getMaxV()).color(r, g, b, a).next();
                tess.draw();
            }
    }
}
