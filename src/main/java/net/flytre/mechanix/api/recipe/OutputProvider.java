...
public class OutputProvider {
    ...
    public OutputProvider(ItemStack stack) {
        this.isTag = false;
        this.stack = stack;
        this.taggedItem = null;
    }

    public OutputProvider(TaggedItem item) {
        this.isTag = true;
        this.stack = null;
    ...
    public ItemStack getStack() {
        return isTag ? taggedItem.getItemStack() : stack;
    }
    ...
    public static OutputProvider fromJson(JsonElement jsonElement) {
        ...
        if(JsonHelper.hasString(json, "tag")) {
            ...
            return new OutputProvider(new TaggedItem(id,i));
        }
        ...
        if(jsonElement.isJsonPrimitive()) {
            ...
            return new OutputProvider(itemStack);
        }

        String string = JsonHelper.getString(json, "item");
        Item item = Registry.ITEM.getOrEmpty(new Identifier(string)).orElse(Items.AIR);
        int i = JsonHelper.getInt(json, "count", 1);
        return new OutputProvider(new ItemStack(item,i));
    }
    ...
    public static OutputProvider fromPacket(PacketByteBuf buf) {
        ...
        if(isTag) {
            return new OutputProvider(new TaggedItem(buf.readIdentifier(),buf.readInt()));
        } else {
            return new OutputProvider(buf.readItemStack());
        }
    }
