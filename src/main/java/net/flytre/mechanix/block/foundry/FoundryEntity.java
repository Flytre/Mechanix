...
public class FoundryEntity extends MachineEntity<DoubleInventory, ItemFluidProcessingRecipe<DoubleInventory>> implements DoubleInventory {
    private final DefaultedList<FluidStack> fluidInventory;

    public FoundryEntity() {
        super(MachineRegistry.FOUNDRY.getEntityType(), RecipeRegistry.CASTING_RECIPE, DefaultedList.ofSize(1, ItemStack.EMPTY),
                (World world, DoubleInventory inventory) -> world.getRecipeManager().getFirstMatch(RecipeRegistry.CASTING_RECIPE, inventory, world).orElse(null)
                , 40);
    ...
    public @Nullable ScreenHandler createMenu(int syncId, PlayerInventory inv, PlayerEntity player) {
        return new FoundryHandler(syncId, inv, this, this.getDelegate());
    }
}
